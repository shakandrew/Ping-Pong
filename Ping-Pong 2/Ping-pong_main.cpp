...
// CONST VARIABLES
const TCHAR szWindowClass[] = _T("Pin-Pong II");//class main window
const TCHAR szTitle[] = _T("Pin-Pong II");//app's title bar
...
void checkBallCrossesTheBorder(HWND hWnd)
{
	...
	if (rect.left < 0 && ball.getMoveSide() < 0)
	{
		if (rect.right > 0)// if ball crosses the border send his coordinates
		{
			if (!ballSent)
			{
	...
	{
		PostMessage(lhWnd, END, NULL, (LPARAM)1);
		PostMessage(hWnd, END, NULL, (LPARAM)1);
	}
}
...
{
	PostMessage(HWND_BROADCAST, RNAME, NULL, (LPARAM)hWnd);
}
...
bool sendBallState(Ball& ball, HWND hParent)
{
	...
	cds.lpData = &ball;
	SendMessage(lhWnd, WM_COPYDATA, (WPARAM)hParent, (LPARAM)&cds);
}
// END
...
int onCopyData(HWND hWnd, HWND hSourceWnd, PCOPYDATASTRUCT cds)
{
	...
	if (cds->cbData == sizeof(ball))
	{
		...
		if (lstrcmp(sourceClassName, TEXT("Pin-Pong I")) == 0)
		{
			//Safe data copy
...
{
	if (!ball.getState() && lhWnd != nullptr) {
		RECT rect = player.getRect();
		ball.setBallParams(true, rect.left - 10 * BALL_RADIUS, rect.top + PLAYER_HEIGHT, 1, 0);
	}
	return 0;
}
int onKeyDown(HWND hWnd, UINT wParam, BOOL smt, int lParam1, UINT lParam2)
{
	...
	if (wParam == VK_UP)
		player.setY(player.getRect().top + 7 * PLAYER_HEIGHT / 10);
	if (wParam == VK_DOWN)
		player.setY(player.getRect().bottom - 7 * PLAYER_HEIGHT / 10);
	ReleaseDC(hWnd, hdc);
	return 0;
}
...
Player::Player()
{
	setPlayerParams(SCREEN_WIDTH - 2 * PLAYER_WIDTH, PLAYER_HEIGHT, PLAYER_WIDTH, PLAYER_HEIGHT);
}
...
Ball::Ball()
{
	setBallParams(false, SCREEN_WIDTH - 3 * PLAYER_WIDTH - BALL_RADIUS, BALL_RADIUS, 0, 0);
}
...
void Ball::collision(RECT playerRect)
{
	if (dX > 0 && Y > playerRect.top && Y < playerRect.bottom &&
		X + BALL_RADIUS - playerRect.left < REFLECTION &&
		X + BALL_RADIUS - playerRect.left > -REFLECTION)
	{
